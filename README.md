...
**This README, every ADR, and all code comments are in Chinese** — see the section below.
The glossary in [`CONTEXT.md`](CONTEXT.md) is in English by design.
...
v1 只接一个来源（红果），目标是能稳定播、自己天天刷。

## 这是什么，不是什么

**做**：短剧库（搜索 + 四个一级标签 + 题材 chip 跳搜索 + 双列海报流 + 「上次观看」续播入口）、
榜单（四个榜 + 下拉刷新 + 分页）、详情页（模糊封面铺底 + 选集 + 评分/热度 + Hero 连贯动画）、
播放页（右竖控制栏、锁屏、选集、收藏、倍速、画质、亮度/音量手势、自动连播）、
我的（收藏 / 历史）、设置页（主题 / 播放 / 手势与控制 / 剧库与存储，**每项都真的接线**）、
本地观看进度与三张 sqflite 表。

**不做**：账号与云同步、弹幕、离线下载、多源（v1 只有红果）、像素级仿红果官方 UI。
这些不是「还没做」，是有意划出去的——理由见 [`docs/plan.md`](docs/plan.md) 的「不在本计划内」。
...
深浅两套主题在设置里切；UI 与深色色值以 `参考的前端页面/` 里的红果截图为依据。
...
# 协议包单独跑测试前，它自己也要解一次依赖
cd packages/hongguo_dart && dart pub get
```
...
个人学习与研究用途的客户端，不隶属于红果，也不分发上游的任何内容。
仓库里的上游接口实现（签名 / 取流 / 解析）只用于协议研究与互操作性验证，不构成任何形式的授权。
截图中的剧集封面与正片画面来自红果，版权归其各自权利人；`参考的前端页面/` 是对其界面的引用性使用。

本仓库**没有 LICENSE 文件**——即默认保留所有权利。这不是遗漏：仓库里混着逆向得来的协议实现
与第三方素材，为整仓库挂一个开源许可等于替那些部分也做了授权，而我没有这个权利。

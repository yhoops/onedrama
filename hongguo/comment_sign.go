...
type CommentNonce [11]byte

func NewCommentNonce() (CommentNonce, error) {
	var nonce CommentNonce
	_, err := rand.Read(nonce[:])
	return nonce, err
}

// SignCommentRequest 给弹幕请求写 X-Gorgon / X-Argus / X-Ladon。
func SignCommentRequest(request *http.Request, nonce CommentNonce, now time.Time) {
	...
	request.Header.Set("X-Argus", commentArgus(query, stamp, nonce))
	request.Header.Set("X-Ladon", commentLadon(request.URL.Query().Get("aid"), stamp, nonce[2:6]))
}

func commentPad(data []byte) []byte {
...
func commentLadon(aid string, timestamp uint32, prefix []byte) string {
	...
	data := commentPad([]byte(strconv.FormatUint(uint64(timestamp), 10) + "-1611921764-3019"))
...
func commentArgus(query string, timestamp uint32, nonce CommentNonce) string {
	...
	bodyHash, queryHash := SM3(make([]byte, 16)), SM3([]byte(query))
	...
	container = commentPad(append(container, 0x56, 0x7b))

...
**contain 是照参考图复刻的**，不是我们自选的口径：量过两张参考截图，视频区是 **0–239 与 2160–2399 纯黑（rmax=0）、240–2159 有画面**，即
...
## 阶段 5 — 搜索记忆 / 剧库导入 / 下一集预取（已完成，发布为 1.1.0+2）
...
**没验到的**：这一屏在六张参考图里**没有对应的**（那六张是设置 / 我的 / 播放 ×2 / 首页 / 详情），所以「最近搜索在上、热门在下」以及长按删的交互都是自由设计，没有可核对的依据。
...
- **没参考可核的**：六张参考图里没有搜索页，5a 的空态是自由设计。

---

## 阶段 6 — 剧库部数与新增 / 切标签流畅 / 榜单预热

### 6a 设置页那一行：部数与本次新增（[ADR-0010](adr/0010-library-count-excludes-the-recommendation-tab.md)）

- [x] **剧库部数** = 分类标签快照去重后的剧 ID 数（`AppDatabase.libraryCount()`，`COUNT(DISTINCT drama_id)`）；**综合不计**——实测它连拉两次第 1 页只有约六成重叠，而分类接口两次完全一致
- [x] **本次新增** = 分类标签里这次新出现的 ID 数。必须在 `replaceLibraryTab` **之前**把旧快照的 ID 读出来做 diff——那一手是先 delete 再 insert
- [x] 右侧文案「`267 部 · 3 分钟前`」：部数占了时间戳的位置，但**时间戳没丢**（快照每次启动都重写，平时它几乎总是「刚刚」；只有导入连续失败才会停在两天前，而那正是最需要知道的时候）。快照为空时仍是「尚未更新」
- [x] 跑完弹一条 SnackBar「本次新增 12 部，剧库共 267 部」；部分失败追加「· 1 个标签没更新上，保留了旧数据」
- [x] **只有点击那一轮弹**，启动那一轮静默——一进 App 就冒东西出来太吵
- [x] **首次导入不弹**（那时「新增」就是全部部数，报出来像是上游一次上了 270 部）；但首次导入有失败要说，否则全新安装下点一次「更新剧库」一点反馈都没有
- [x] 启动那一轮跑完时，设置页那一行**自动刷新**：它的进度走同一个 notifier，但跑完之后的部数与时间戳是 `_refreshUsage()` 取的快照、不是实时的。用户正停在设置页时不给这一下，那一行会一直停在旧数字上
- [x] 文案抽成顶层函数 `importToast`，三条分支（首次不弹 / 正常 / 部分失败）有单测钉住
- [x] `CONTEXT.md` 新增 **Library Count** / **Newly Added** 两个词

### 6b 切标签不闪：四个标签页 keepAlive

`PageView` 默认把滑走的页整个 dispose，切回来整棵树重建——封面重走「磁盘读 → 解码 → `DramaCover` 那 220ms 淡入」，那一下闪就是它。**数据层其实早就不卡了**（启动导入器已经在拉这四个标签并落快照，`LibraryFeed` 是先读快照秒画再拉网络），卡的是封面。

- [x] `_FeedViewState` 加 `AutomaticKeepAliveClientMixin`（`wantKeepAlive => true`）：切回来不重建、封面不重解码、滚动位置也留住（底部三页已经是这个口径，见 `home_shell.dart`）
- [x] 封面磁盘缓存对象数上限 **200 → 400**（`CoverCacheManager`）：导入器一次热 360 张，而 `DefaultCacheManager` 默认只留 200 个，约 **160 张刚热完就被 LRU 挤掉、白热**。360 张约 8.6 MB，提到 400 的磁盘代价可以忽略
- [x] **四处共用同一个 manager**（`DramaCover` / `CoverPrefetcher` / `openDrama` 的 `precacheImage` / 导入器预热）——各开各的会让预热热到别的缓存里去。它**不进内存 `ImageCache` 的键**（`CachedNetworkImageProvider.==` 不比 `cacheManager`），所以「照抄同一个构造就能命中」那条规则不受影响
- [x] 改缓存键留下的孤儿（旧的 `libCachedImageData`）在「清除缓存」里一起清、也一起算进占用，不留着占地方

**刻意没动内存 `ImageCache` 上限**（Flutter 默认 100 MB）。keepAlive 之后四个标签各自的最后一屏都留在内存里、约等于四倍单标签开销，而 `drama_card.dart` 里那段注释算过「一遍正常使用约 92 MB」——已经很贴上限。**这个数真机上要量**，量出来不够再调。

### 6c 启动预热榜单

- [x] 启动后后台把四个榜的第 1 页刷进 `RankingCache`（`RankingWarmer`）。榜单页**一行都没改**——它现有的 `_restoreFromCache()` 会自动秒开
- [x] 每榜试 **3** 次（页面自己是 6 次）、四个榜并发；走 web 主机，与剧库导入走的 app 主机不是同一路，不抢接口
- [x] 缓存比 **30 分钟**还新就不重刷（榜单每日更新，而用户一天里可能反复启动；清缓存会把榜单缓存一起清掉，所以清完之后这条不挡重新预热）
- [x] **只热数据、不热封面**：内存里的封面会被首页滑动的封面挤掉（`ImageCache` 是 LRU），落盘的数据不会
- [x] 顺带修掉 `/rank/` 的缓存缺陷：`_settle` 只看 `statusCode == 200` 就入缓存，而红果榜单约一半请求返回「新渲染」那版**没有数据的 200**——那份坏 HTML 被钉住五分钟，`_fetchWithRetry` 的后五次重试全部命中它、必然失败（重试只在非 200 上有效）。把 `/rank/` 移出 `isCacheableRequest` 白名单之后，重试才真的在重试

### 6d 首访卡顿：启动后预热另外三个标签的首屏封面

6b 的 keepAlive 只让**重复**切换不卡；**第一次**进某个标签仍要从零构建那一页、第一次解码它的封面。这一节削的就是这一下。

**先量，再改**：把设备上真实快照的一个标签（86 行、94 KB payload）拿去解析（走 App 里同一条 `decodeJsonObject` + `Drama.fromJson`）——**2–5 ms**，而且是 JIT 的上界（真机 AOT 更快）；读库那次也是异步的。所以「启动时把四个标签的**数据**加载好」解决不了这件事。卡的是封面：约 4–6 张海报要在 260 ms 的转场里走完「磁盘读 → 解码 → 显存上传」，还要叠 220 ms 淡入。

- [x] `CoverPrefetcher` 加 `warmFrom(index, urlAt)`：语义是「从第 index 张起热一个窗口」。**不能复用 `advanceTo`** —— 它取的是「前缘**之后**」，传 0 会被理解成「构建到了第 0 条」而跳过首屏第一张
- [x] `CoverPrefetcher.firstScreen = 6`（2 列网格一屏约 4–6 张），**刻意比滚动预取的 12 小**：超出首屏的那些切过去也不会立刻看到，热了是白占内存
- [x] `_LibraryPageState` 第一帧之后，按标签下标顺序**串行**热另外三个标签的首屏；串行是为了同时最多只有 3 张在飞，让当前标签（综合）的首屏先赢——它要抢同一批解码与显存上传
- [x] **先 `ensureLoaded` 再热**：`LibraryFeed` 会把新拉到的那一页插到最前（`absorbDramas` 的 `atTop`），只按快照热的话，恰好在新剧上架时热的会是被挤下去的旧那几张
- [x] 跳过当前标签（它的封面本来就在画）；`dispose` 时一起 dispose

**「先加载三个 feed」的净请求代价约 0**：`/reading/distribution/category/` 在 TTL 缓存白名单里（`network.dart`），feed 在 t≈0 拉的第一页会被缓存 5 分钟，导入器 t≈3.5s 拉同一页时直接命中——那 3 个请求只是**提前**打掉导入器的 3 个。

**边界**：这套只在**快照非空**时有活干。全新安装或清缓存后第一次启动，三个标签的快照是空的、`ensureLoaded` 走网络，首屏仍然会有那一下——那时没有任何东西可预热。这是「本地优先」的固有代价，不是遗漏。

**内存实测（Redmi K60，release 包）**：

| 状态 | TOTAL PSS | Native Heap | Graphics |
| --- | --- | --- | --- |
| 启动后，只开过综合 | 169 MB | 33 MB | 64 MB |
| 四个标签都走过一遍（6b 的 keepAlive 之后） | 355 MB | 171 MB | 80 MB |

多出来的 186 MB 里，原生堆 +138 MB 就是解码后的封面。**它会超过 `ImageCache` 那 100 MB 上限**，因为上限只管**能被淘汰**的那部分，而 keepAlive 让四个标签的封面仍被 widget 引用、属于「活图」，既不计入上限也挤不掉。**有界、不是泄漏**：`SliverGrid` 仍会回收滚出屏幕的卡，每个标签常驻的只是视口附近那一屏（约 8–12 张）。6d 之后预期**地板**抬到约 300 MB，而峰值不变。

### 6e 去掉每次启动的自动导入（[ADR-0012](adr/0012-library-snapshot-only-on-first-launch.md)）

> **这一节落在 1.2.0+3 打包之后**，所以那个 APK 里还是「每次启动都导」——下一个包才含它。

[ADR-0008](adr/0008-library-snapshot-on-every-launch.md) 的口径是「每次启动自动拉四个标签各 5 页」，代价是每次启动固定 20 次签名请求 + 350 多张封面（约 8.6 MB）、跑十几秒——而**绝大多数启动都白花**：上游的上架节奏是每天几十部，不是每次启动几十部。

真机上还撞到更硬的一堵墙：一小时里连跑四轮导入之后，有一轮**四个标签全挂**（`逐标签 0:-1 1:-1 2:-1 3:-1`）；同一时刻用 Go 参考实现、换个设备号打同一个接口，549 ms 就拿到了数据——所以那一挂是**设备级的风控/限流**，不是上游故障，也不是代码坏了。几分钟后再启动恢复正常（`落盘 349 部 · 新增 6 部`）。

- [x] `LibraryImporter` 加 prefs 标记 `library_startup_import_done`：`startupImportDone()` / `markStartupImportDone()`
- [x] `main.dart` 只在标记未置位时跑那一轮；**先置位再跑，失败也算跑过**
- [x] `forgetImportedAt()` **刻意不碰**这个标记——清缓存只清快照与时间戳，于是清完缓存下次启动不会被自动填回去（那正是用户清缓存想避免的）
- [x] 首次导入失败留一条 `[library] 首次导入失败：…` 日志，不在用户面前冒任何东西
- [x] `CONTEXT.md` 的 **Library Snapshot** 里「Rewritten wholesale on every app start」改成「once automatically on the very first launch, thereafter only when the user asks for one」
- [x] ADR-0008 加状态说明：**部分被取代**（「每次启动」那半条作废，「首页本地优先」那半条不变）

**后果（都写进 ADR-0012 了）**：

- 剧库会变旧直到用户点 → **设置页那行的时间戳成了唯一的新鲜度指示器**（这正是当初没让它被部数取代的理由）。
- 清缓存之后不再自动拉 → 本地没快照时首页仍可用（`LibraryFeed` 走网络），但**断网时首页是空的**，要手动点一次「更新剧库」。
- 首次导入失败也要手动点。
- 首次启动那一次封面预热无活可干（快照是空的），那一次的首次切换仍会卡一下。
- 顺带减轻了「本次新增」虚高那个已知问题（导入间隔拉长后真实上新占比变高），但**没修掉**——漫剧那一支的旋转仍在。

### 怎么验（6a / 6b / 6c / 6d / 6e）

- `flutter analyze` 干净；`flutter test` **56 条全绿**（新增 11 条：分类标签口径 2 条 + 弹窗文案 6 条 + 预热窗口 3 条）。
- **sqflite 在单测里跑不了**，所以 `libraryCount()` / `libraryTabIds()` 与那两条 SQL 只能真机验——那条 SQL 用本机 `sqlite3` 建了同构的表验过（造一部剧同时出现在真人剧与漫剧里：`COUNT(DISTINCT)` 得 5、按行数得 6、把综合算进来得 7），但**走 sqflite 那一层没验**。

**真机（Redmi K60 / 23013RK75C，release 包 1.1.0+2，UI 交互手测、日志与 `dumpsys` 取证）**：

| 验的什么 | 证据 |
| --- | --- |
| 导入 + 新增口径 | `[library] 导入完成 · 落盘 355 部 · 新增 265 部（首次，没有上一份快照） · 逐标签 0:90 1:85 2:90 3:90 · 封面 337 张 · 14s`——**新增 265 = 85+90+90**（三个分类标签），而落盘 355 含综合的 90，**综合确实没进新增**；`firstImport` 也正确地是 true（全新安装） |
| 榜单预热 | `[ranking] 预热完成 · 4/4 个榜 · 2s`——修 `/rank/` 缓存之前，那 6 次重试是白打的 |
| 6a 交互 | 手测通过：右侧部数、SnackBar 文案、再点一次新增归零，都对 |
| 6b 切标签 | **重复切换不卡了**，滚动位置留住；但**首次进某个标签仍卡一下**——这正是 6d 要削的那一下 |
| 6d 首访卡顿 | 手测通过：冷启动后等预热铺好再切四个标签，**首访那一下没了**（`[warm] 首屏预热排队 · 逐标签 1:6 2:6 3:6` 每轮都跑到） |
| 6e 只跑一次 | 装完后连启动 5 次：第 1 次跑了那一轮（**由标记被置位反证**——`[library] 首次导入已跑过，跳过自动更新` 在之后 4 次启动里都出现，而置位只有 `_importLibraryOnce` 会做），之后 4 次都不再导入（直接观测）。**第 1 次那一轮的完成日志没抓到**：封面要重下，跑得比 26 秒的抓取窗口久 |
| 6b 内存 | 只开过综合 **169 MB** PSS → 四个标签都走过 **355 MB**（表见 6d） |
| Flutter 异常 | 无（logcat 里只有 MIUI 的 dex 缓存噪音） |

**顺带修掉的构建问题**：`build.gradle.kts` 原来只给 `release` 配了签名，debug 变体仍用 debug 密钥，两者互相覆盖安装会 `INSTALL_FAILED_UPDATE_INCOMPATIBLE`、只能卸载（风险 7 的根因）。现在两个 buildType 共用同一把签名，**debug 与 release 可以互换**，以后再不用卸载。代价是本次要先卸一次（已备份 `onedrama.db` 与 `FlutterSharedPreferences.xml` 到 `onedrama-device-backup-20260922/`）。

**已知问题（本轮未修）：「本次新增」被上游的窗口旋转系统性虚高**

三轮冷启动（间隔一分钟）分别报 **176 / 54 / 55 / 36** 部新增，而上游不可能每分钟上架四五十部短剧。探针（间隔 2 秒连拉两遍、各 5 页）：
...
漫剧那一支会整体换一批，而且**不稳定**：同一个探针再跑一次就变成 90/90 全稳；逐页测则是第 1–4 页各 18/18 稳定、第 5 页 0/18 全换。机制未定论（漫剧目录只有约 36 部、后面几页是越界填充？还是像榜单那样多版本摇摆？）。

**候选修法**（待定）：① 新增只算各标签第一页——分类接口按 `online_time` 排序，真上新必然出现在头部，而第 1 页实测是稳的（代价：很久不导入时最多只报 3×18 = 54 部）；② 落一张「见过的剧 ID」表（建库 2 → 3），新增 = 从来没见过的 ID——窗口怎么转都无所谓，但集合会持续变大。

**没验到的**：
- 6e 的**全新安装**那一趟没有直接观测：首次进入 App 的导入是间接反证的（标记被置位 ⟹ 那一支跑过），而「之后不再导入」是直接观测的。要直接看，得先卸载再装——那会清掉设备上的快照与 prefs，所以没做。
- 6d 的**两条边界**没验：① 冷启动后 3.5 秒内切标签（预热要等三个 feed 的网络加载，那时还没铺好）——预期仍会卡一下，这是方案的固有边界；② 预热有没有把**综合的首屏**拖慢（它俩抢同一批解码与显存上传）。两条都要靠眼睛看，没有客观手段。
- 6d 的内存代价：封面源图是 **400px 宽的 HEIC**（URL 里 `aifit:400:0`），`gridCoverWidth` 那个 480 比源图还大、缩不下来，所以每张约 1.14 MB；而 `dumpsys meminfo` 对 Impeller 的纹理记账不完整（预热 18 张只量到 +8 MB，与估算对不上），**这个数没有可靠手段量**，只能靠日志确认排了多少张。
- 部分失败那条 SnackBar 分支只有单测，真机上没造出「只挂一个标签」的情形（与 5b 同一个坑）。
